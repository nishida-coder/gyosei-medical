...
const OUT_DIR = 'C:/libunworks/projects/gyosei-medical/qa-screenshots/20260423-v1.49';
const PORT = 9229;
const HEIGHT = 2500;
const WAIT_MS = 6000;

const JOBS = [
  { slug: 'home_news',  url: 'https://gyosei-medical.com/',               width: 390 },
  { slug: 'home_news',  url: 'https://gyosei-medical.com/',               width: 430 },
  ...
  { slug: 'dr-class',   url: 'https://gyosei-medical.com/class-clinic/',  width: 768 },
];

// dedup: home_news appears twice but with different widths -> filename includes width, OK.
...
async function captureOne(job, browserCdp) {
  // Create fresh target per job via Target.createTarget on the browser endpoint.
  const { targetId } = await browserCdp.send('Target.createTarget', { url: 'about:blank' });
  const { sessionId } = await browserCdp.send('Target.attachToTarget', { targetId, flatten: true });
  // Build a session-scoped CDP wrapper using the browser WS with sessionId tagging.
  const cdp = {
...
async function main() {
  ...
  // Connect to browser-level endpoint to drive Target.* over a single socket.

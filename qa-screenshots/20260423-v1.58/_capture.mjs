// CDP screenshots + doctor-card centering + ol#post_list2 display check
// Two targets: kokoro (1280x1800) and class (390x2500).
...
const OUT_DIR = 'C:/libunworks/projects/gyosei-medical/qa-screenshots/20260423-v1.58';
const PORT = 9238;
const WAIT_MS = 6000;

const TARGETS = [
  { name: 'kokoro_1280', url: 'https://gyosei-medical.com/kokoromental/', width: 1280, height: 1800 },
  { name: 'class_390',   url: 'https://gyosei-medical.com/class-clinic/',  width: 390,  height: 2500 },
];
...
const MEASURE_EXPR = `(() => {
  ...
  // ol#post_list2 computed display
  const postList = document.querySelector('ol#post_list2');
  let postListInfo = { found: false };
  if (postList) {
    const cs = getComputedStyle(postList);
    postListInfo = {
      found: true,
      display: cs.display,
      gridTemplateColumns: cs.gridTemplateColumns,
      childCount: postList.children.length,
    };
  }
  return JSON.stringify({ card: cardInfo, postList: postListInfo });
})()`;
